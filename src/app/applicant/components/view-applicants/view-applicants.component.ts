          ...
          JobData.applicantApplied.forEach((applicantID: any) => { 
            ...
            if (applicantID ) { 
              ...
              this.data.getApplicant(applicantID).then(ApplicantData => { 
                ...
                if(!this.JobList.includes(JobData)){ 
              ...
              if(!this.check.includes(applicantID)){  
                ...
                // Push the applicant data into the array 
...
FilterByKeyword(event: any) {
  ...
  const keyword = event.target.value;
   this.filteredApplicant = this.apllicant.filter(applicant => 
   applicant.firstName.toLowerCase().includes(keyword.toLowerCase()) || 
   applicant.lastName.toLowerCase().includes(keyword.toLowerCase()) 
   );
   this.isfilterd= true;
}
